package manager;

import model.Book;
import model.NovelBook;
import model.ScienceBook;

import java.util.Scanner;


public class BookManager {


    public Book[] createListBook(Scanner scanner) {
        System.out.println("Nhập vào số lượng sách cần tạo: ");
        int size = scanner.nextInt();
        return new Book[size];
    }

    public Book createBook(Scanner scanner) {
        System.out.println("Nhập vào tên sách: ");
        String name = scanner.nextLine();
        System.out.println("Nhập vào đơn giá sách: ");
        long price = scanner.nextLong();
        System.out.println("Nhập vào số lượng của sách: ");
        int quantity = scanner.nextInt();
        scanner.nextLine();
        System.out.println("Nhập vào ngày tạo của sách: ");
        String date = scanner.nextLine();
        return new Book(name, price, quantity, date);
    }

    public void displayAllBook(Book[] books) {
        System.out.println("Tất cả sách là: ");
        for (Book book : books) {
            System.out.println(book);
        }
        System.out.println("---------------------");
    }

    public double sumPrice(Book[] books) {
        System.out.println("Tổng giá của tất cả sách là: ");
        double sumPrice = 0;
        for (Book book : books) {
            sumPrice += (book.getPrice() * book.getQuantity());
        }
        System.out.println(sumPrice);
        System.out.println("---------------------");
        return sumPrice;
    }

//    public void maxPrice(Book[] books) {
//        System.out.println("Quyển sách có giá tiền cao nhất là: ");
//        long maxPrice = books[0].getPrice();
//        Book myBook = books[0];
//        for (Book book : books) {
//            if (maxPrice < book.getPrice()) {
//                maxPrice = book.getPrice();
//                myBook = book;
//            }
//        }
//        System.out.println(myBook);
//        System.out.println("---------------------");
//    }

    public double maxPrice(Book[] books) {
        System.out.println("Quyển sách có giá tiền cao nhất là: ");
        double maxPrice = books[0].getPrice();
        for (Book book : books) {
            if (maxPrice < book.getPrice()) {
                maxPrice = book.getPrice();
            }
        }
        for (Book book : books) {
            if (book.getPrice() == maxPrice) {
                System.out.println(book);
            }
        }
        return maxPrice;
    }

//    public void minPrice(Book[] books) {
//        System.out.println("Quyển sách có giá tiền thấp nhất là: ");
//        long minPrice = books[0].getPrice();
//        Book myBook = books[0];
//        for (Book book : books) {
//            if (minPrice > book.getPrice()) {
//                minPrice = book.getPrice();
//                myBook = book;
//            }
//        }
//        System.out.println(myBook);
//        System.out.println("---------------------");
//    }

    public void minPrice(Book[] books) {
        System.out.println("Quyển sách có giá tiền thấp nhất là: ");
        double minPrice = books[0].getPrice();
        for (Book book : books) {
            if (minPrice > book.getPrice()) {
                minPrice = book.getPrice();
            }
        }
        for (Book book : books) {
            if (book.getPrice() == minPrice) {
                System.out.println(book);
            }
        }
        System.out.println("---------------------");
    }

    public void searchByType(Book[] books, String searchType) {
        boolean check = false;
        for (Book book : books) {
            if (book instanceof ScienceBook) {
                if (((ScienceBook) book).getType().equals(searchType)) {
                    check = true;
                    System.out.println(book);
                }
            }
        }
        if (!check) {
            System.out.println("Không có quyển sách nào có thể loại đó!!!");
        }
        System.out.println("---------------------");
    }


    public void searchByAuthor(Book[] books, String searchAuthor) {
        boolean check = false;
        for (Book book : books) {
            if (book instanceof NovelBook) {
                if (((NovelBook) book).getAuthor().equals(searchAuthor)) {
                    check = true;
                    System.out.println(book);
                }
            }
        }
        if (!check) {
            System.out.println("Không có quyển sách nào của tác giả đó!!!");
        }
        System.out.println("---------------------");
    }

    public void averagePriceOfScienceBook(Book[] books) {
        long average = 0;
        int count = 0;
        for (Book book : books) {
            if (book instanceof ScienceBook) {
                average += book.getPrice();
                count++;
            }
        }
//        System.out.println("Đơn giá trung bình của sách khoa học là: ");
        System.out.println(average / count);
        System.out.println("---------------------");
    }

    public void rangePrice(Book[] books, long minPrice, long maxPrice) {
        boolean check = false;
//        System.out.println("Các quyển sách trong khoảng giá là: ");
        for (Book book : books) {
            if (book.getPrice() >= minPrice && book.getPrice() <= maxPrice) {
                check = true;
                System.out.println(book);
            }
        }
        if (!check) {
            System.out.println("Không có quyển sách nào trong khoảng giá vừa nhập!!!");
        }
        System.out.println("--------------------------");
    }

    public void searchByPrice(Book[] books, long price) {
        boolean check = false;
//        System.out.println("Các quyển sách có giá tiền vừa nhập là: ");
        for (Book book : books) {
            if (book.getPrice() == price) {
                check = true;
                System.out.println(book);
            }
        }
        if (!check) {
            System.out.println("Không có quyển sách nào có giá vừa nhập!!!");
        }
        System.out.println("--------------------------");
    }

    public void displayScienceBook(Book[] books) {
        boolean check = false;
//        System.out.println("Các quyển sách khoa học là: ");
        for (Book book : books) {
            if (book instanceof ScienceBook) {
                check = true;
                System.out.println(book);
            }
        }
        if (!check) {
            System.out.println("Không có quyển sách nào thuộc thể loại khoa học!!!");
        }
        System.out.println("--------------------------");
    }

    public void displayNovelBook(Book[] books) {
        boolean check = false;
//        System.out.println("Các quyển sách tiểu thuyết là: ");
        for (Book book : books) {
            if (book instanceof NovelBook) {
                check = true;
                System.out.println(book);
            }
        }
        if (!check) {
            System.out.println("Không có quyển sách nào thuộc thể loại tiểu thuyết!!!");
        }
        System.out.println("--------------------------");
    }
}
